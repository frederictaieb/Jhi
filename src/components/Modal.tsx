...
const Modal: React.FC<ModalProps> = ({ 
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      if (modalRef.current && !modalRef.current.contains(e.target as Node) && !isSessionActive) {
        onClose();
      }
    };
  ...
  }, [isOpen, onClose, isSessionActive]);
  ...
  return (
    <div className="modal-backdrop fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
      <div 
        className={`modal-container bg-white dark:bg-gray-800 rounded-lg shadow-lg max-h-[90vh] overflow-hidden ${
          isSessionActive ? 'w-[90vw] max-w-5xl' : 'w-[90vw] max-w-2xl'
        }`} 
        ref={modalRef}
      >
        {!isSessionActive ? (
